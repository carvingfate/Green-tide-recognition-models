...
for i in range(len(path_list1)):
        ...
        for j in range(temp1.shape[0]):
                for k in range(temp1.shape[1]):
                        if temp1[j][k][0]+temp1[j][k][1]+temp1[j][k][2]+temp1[j][k][3] in my_set:
                            continue
                        #print(str(temp1[j][k][0])+','+str(temp1[j][k][1])+','+str(temp1[j][k][2])+','+str(temp1[j][k][3])+','+str(temp2[j][k][0])+'\n')
                        if temp2[j][k][0] > 122:
                            temp2[j][k][0] = 255
                        else:
                            temp2[j][k][0] = 0
...
sklearn.metrics.f1_score(test_label, pred_label, labels=None, pos_label=1)
#保存模型
#以二进制的方式打开文件：
